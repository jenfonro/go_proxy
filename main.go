...
func main() {
	...
	// Example: GET /api/speed?bytes=2097152
	speedPath := mountPath(basePath, "/api/speed")
	...
	proxyPrefix := mountPath(basePath, "/spider/proxy/baidu/")
	mux.HandleFunc(proxyPrefix, func(w http.ResponseWriter, r *http.Request) {
		...
		token := strings.TrimPrefix(r.URL.Path, proxyPrefix)
		token = strings.Trim(token, "/")
		// Enforce "no suffix" tokens to avoid misleading the client about container/codec.
		...
		// Safety: only allow Baidu pcs domains for this route.
		hostLower := strings.ToLower(u.Hostname())
		if hostLower != "baidupcs.com" && !strings.HasSuffix(hostLower, ".baidupcs.com") {
			http.Error(w, "Invalid url host", http.StatusBadRequest)
			return
		}

		tw := &trackedWriter{ResponseWriter: w}
		if err := proxyStream(client, tw, r, target, e.Headers, r.Method == http.MethodHead); err != nil {
	...
	quarkProxyPrefix := mountPath(basePath, "/spider/proxy/quark/")
	mux.HandleFunc(quarkProxyPrefix, func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodOptions {
			writeCORSHeaders(w)
			w.WriteHeader(http.StatusNoContent)
			return
		...
		token := strings.TrimPrefix(r.URL.Path, quarkProxyPrefix)
		token = strings.Trim(token, "/")
		if strings.Contains(token, ".") {
			http.Error(w, "Not Found", http.StatusNotFound)
			return
	...
	// CatPawOpen -> register a token for a Baidu URL.
	mux.HandleFunc(mountPath(basePath, "/api/baidu/register"), func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodOptions {
			writeCORSHeaders(w)
			w.WriteHeader(http.StatusNoContent)
			return
	...
	// CatPawOpen -> register a token for a Quark direct URL (already resolved by CatPawOpen).
	mux.HandleFunc(mountPath(basePath, "/api/quark/register"), func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodOptions {
			writeCORSHeaders(w)
			w.WriteHeader(http.StatusNoContent)
			return
